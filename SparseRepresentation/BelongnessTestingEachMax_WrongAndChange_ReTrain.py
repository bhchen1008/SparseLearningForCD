...
#PairedLearnerPath
plPath = sys.argv[len(sys.argv)-2]
#PairedLearnerOutput
plOutput = sys.argv[len(sys.argv)-1]
...
#        if((meanCompare-currentMean) > 2*stdCompare):
        ...
        if(currentMean < changeThreshold ):
            ...
            if(currentMean<changeThreshold):
                ...
                PLDataW = open("D:/NetDB/LAB/research/ICDM-2014/dataset/RealData/sensor_h0-1_New/PLData.arff",'w')
                PLDataW.write(testLoader.arffAttribute+'\n@data\n')
                for i in range(instNo,numOfInsts):
                    PLDataW.write(testLoader.transactionList[i]+'\n')
                PLDataW.close()
                numAlgoWindow = 1000
                os.system("java -jar " + plPath + " D:/NetDB/LAB/research/ICDM-2014/dataset/RealData/sensor_h0-1_New/PLData.arff " + str(numAlgoWindow) + " 0.03 weka.classifiers.trees.J48 " + plOutput)
                break
            else:
